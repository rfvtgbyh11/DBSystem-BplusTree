...
void dfs(off_t node){
    ...
    p = load_page(node);
    printf("\nnode offset: %ld, ", node);
    printf("parent_offset: %ld, ", p->parent_page_offset);
    printf("is_leaf: %d, number_of_keys: %d, ", p->is_leaf, p->num_of_keys);
    printf("next_offset: %ld, ", p->next_offset);
...
int db_insert(int64_t key, char * value) {
    ...
    // insert redistribution
    int prime_key;
    off_t left_offset, right_offset, parent_offset;
    left_offset = 0;
    right_offset = 0;
    ...
    if (parent->next_offset == leaf){
        prime_key = -1;
        right_offset = parent->b_f[0].p_offset;
    }else {
        if (prime_key == 0) left_offset = parent->next_offset;
        if (prime_key > 0) left_offset = parent->b_f[i-1].p_offset;
        if (prime_key < parent->num_of_keys - 1) right_offset = parent->b_f[i+1].p_offset;
    }

    page * left_page = NULL;
    page * right_page = NULL;
    if (left_offset != 0) left_page = load_page(left_offset);
    if (right_offset != 0) right_page = load_page(right_offset);

    // do if redistribute-able
    if (left_page != NULL && left_page->num_of_keys < LEAF_MAX) insert_into_leaf_redis(left_offset, parent_offset, leaf, prime_key, 1, nr);
    else if (right_page != NULL && right_page->num_of_keys < LEAF_MAX) insert_into_leaf_redis(right_offset, parent_offset, leaf, prime_key, 0, nr);
    else insert_into_leaf_as(leaf, nr);

    free(leafp);
    free(parent);
    if (left_offset != 0) free(left_page);
    if (right_offset != 0) free(right_page);
...
off_t insert_into_leaf_redis(off_t neighbor_off, off_t parent_off, off_t redis_off, int prime_key, int is_left, record inst){
    page *parent, *nbor, *redis;
    parent = load_page(parent_off);
    nbor = load_page(neighbor_off);
    redis = load_page(redis_off);
    int i;

    //printf("leaf redistributed is_left = %d, ", is_left);
    //printf("prime key : %d\n", prime_key);

    if (is_left == 1){ // if redistribute with left neighbor node
        nbor->records[nbor->num_of_keys] = redis->records[0]; // make one leftmost key empty
        (nbor->num_of_keys)++;

        for (i = 0; i < redis->num_of_keys - 1; i++){
            if (redis->records[i+1].key > inst.key) break;
            redis->records[i] = redis->records[i+1];
        }
        redis->records[i] = inst;
        
        parent->b_f[prime_key].key = redis->records[0].key;
    }else { // if redistribute with right neighbor node
        for (i = nbor->num_of_keys - 1; i >= 0; i--){
            nbor->records[i+1] = nbor->records[i];
        }
        
        // if new record should be inserted in nbor (new record's key > redis's rightmost record's key)
        if (redis->records[redis->num_of_keys - 1].key < inst.key){
            nbor->records[0] = inst; // insert new record to neighbor
        }else {
            nbor->records[0] = redis->records[redis->num_of_keys - 1];
            for (i = redis->num_of_keys - 1; i > 0; i--){
                if (redis->records[i-1].key < inst.key) break;
                redis->records[i] = redis->records[i-1];
            }
            redis->records[i] = inst;
        ...
        (nbor->num_of_keys)++;
        parent->b_f[prime_key + 1].key = nbor->records[0].key;
    }

    pwrite(fd, parent, sizeof(page), parent_off);
    pwrite(fd, nbor, sizeof(page), neighbor_off);
    pwrite(fd, redis, sizeof(page), redis_off);
    free(parent);
    free(nbor);
    free(redis);
    return redis_off;
...
off_t insert_into_parent(off_t old, int64_t key, off_t newp) {
    ...
    if (bumo == hp->rpo) return insert_into_internal_as(bumo, left_index, key, newp);

    // insert redistribution
    int prime_key;
    off_t left_offset, right_offset, grandp_offset;
    left_offset = 0;
    right_offset = 0;
    grandp_offset = parent->parent_page_offset;
    // find left & right neighbor
    page * grandp = load_page(grandp_offset);
    int i;
    for(i = 0; i < grandp->num_of_keys; i++){
        if (grandp->b_f[i].p_offset == bumo) {
            prime_key = i;
            break;
        }
    }
    if (bumo == grandp->next_offset){
        prime_key = -1;
        right_offset = grandp->b_f[0].p_offset;
    }
    else{
        if (prime_key == 0) left_offset = grandp->next_offset;
        if (prime_key > 0) left_offset = grandp->b_f[i-1].p_offset;
        if (prime_key < grandp->num_of_keys - 1) right_offset = grandp->b_f[i+1].p_offset;
    }
    page * left_page = NULL;
    page * right_page = NULL;
    if (left_offset != 0) left_page = load_page(left_offset);
    ...
    // do if redistribute-able
    if (left_page != NULL && left_page->num_of_keys < LEAF_MAX) insert_into_internal_redis(left_offset, grandp_offset, bumo, prime_key, 1, key, newp);
    else if (right_page != NULL && right_page->num_of_keys < LEAF_MAX) insert_into_internal_redis(right_offset, grandp_offset, bumo, prime_key, 0, key, newp);
    else return insert_into_internal_as(bumo, left_index, key, newp);
...
off_t insert_into_internal_redis(off_t neighbor_off, off_t grandp_off, off_t bumo_off, int prime_key, int is_left, int64_t key, off_t newp_off){
    page *nbor, *grandp, *bumo, *new_p, *child;
    nbor = load_page(neighbor_off);
    grandp = load_page(grandp_off);
    bumo = load_page(bumo_off);
    new_p = load_page(newp_off);
    int i;

    //printf("internal redistributed is_left = %d\n", is_left);
    //printf("prime key : %d\n", prime_key);
    if (is_left == 1){ // if redistribute with left neighbor node
        nbor->b_f[nbor->num_of_keys].key = grandp->b_f[prime_key].key;
        nbor->b_f[nbor->num_of_keys].p_offset = bumo->next_offset;
        child = load_page(bumo->next_offset);
        child->parent_page_offset = neighbor_off;
        pwrite(fd, child, sizeof(page), bumo->next_offset);
        free(child);
        grandp->b_f[prime_key].key = bumo->b_f[0].key;
        bumo->next_offset = bumo->b_f[0].p_offset;
        for (i = 0; i < bumo->num_of_keys - 1; i++){
            if (bumo->b_f[i+1].key > key) break;
            bumo->b_f[i] = bumo->b_f[i+1];
        }
        bumo->b_f[i].key = key;
        bumo->b_f[i].p_offset = newp_off;
        new_p->parent_page_offset = bumo_off;
    }else { // if redistribute with right neighbor node
        for (i = nbor->num_of_keys - 1; i >= 0; i--){
            nbor->b_f[i + 1] = nbor->b_f[i];
        }
        nbor->b_f[0].key = grandp->b_f[prime_key + 1].key;
        nbor->b_f[0].p_offset = nbor->next_offset;
        if (bumo->b_f[bumo->num_of_keys - 1].key < key){ // if splitted child node was on the last index of parent
            nbor->next_offset = newp_off;
            grandp->b_f[prime_key + 1].key = key;
            new_p->parent_page_offset = neighbor_off;
        }else {
            nbor->next_offset = bumo->b_f[bumo->num_of_keys - 1].p_offset;
            child = load_page(bumo->b_f[bumo->num_of_keys - 1].p_offset);
            child->parent_page_offset = neighbor_off;
            pwrite(fd, child, sizeof(page), bumo->b_f[bumo->num_of_keys - 1].p_offset);
            free(child);
            grandp->b_f[prime_key + 1].key = bumo->b_f[bumo->num_of_keys - 1].key;
            for (i = bumo->num_of_keys - 1; i > 0; i--){
                if (bumo->b_f[i-1].key < key) break;
                bumo->b_f[i] = bumo->b_f[i-1];
            }
            bumo->b_f[i].key = key;
    ...
    pwrite(fd, nbor, sizeof(page), neighbor_off);
    pwrite(fd, grandp, sizeof(page), grandp_off);
    pwrite(fd, bumo, sizeof(page), bumo_off);
    pwrite(fd, new_p, sizeof(page), newp_off);
...
int db_delete(int64_t key) {
    ...
    // delete if the key exist
    page *check = load_page(deloff);
    int i;
    for (i = 0; i < check->num_of_keys; i++){
        if (check->records[i].key == key){
